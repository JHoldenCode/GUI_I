...
form.onsubmit = function(e) {
    ...
    e.preventDefault();
    createTableIfValid();
    // set the cap of tabs at 10
    // +1 to the size because this check happens before the tab is added
    if ($("#numberForm").valid() && $("#myTabs >ul >li").size() + 1 <= 10) {
        tableCreated = false;
        addTableToTabs();
        // readds the table to scrollableTable after it is removed in addTableToTabs
        createTableIfValid();
    }
}

function createTableIfValid() {
